...
export function getReqModel(models) {
  if (!models || !models || models <= 0) {
    return
  }
  const param = models.params
  models.filterList.forEach((element) => {
    for (const [key, item] of Object.entries(element)) {
      ...
      if (item === 'daterange' && !element[element.dateProp]) {
        param[element.props[0]] = null
        param[element.props[1]] = null
      }
      if (key === 'prop') {
        param[element.prop] = element[element.prop]
      }
    }
  })
  if (!param.currentPage) {
    param.currentPage = models.resData.currentPage
  }
  if (!param.pageSize) {
    param.pageSize = models.resData.pageSize
  }
  return param
}
 ...
 * 收集数组中的某个属性
 */
export function collectParam(arr, name) {
  var result = []
